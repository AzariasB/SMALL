...
/**
 * A class to link names used in a SMALL program to their description as used by
 * Jasmin.
 *
 * For each name used in the program we store what it becomes to the assembler
 * and the name of it's type.
 *
 *
...
public class Descriptor {
    ...
    /**
     * Static method to create a Value. Does little but included for consistency
     * with other subclasses of Descriptor.
     *
     * @param value Value to be stored in the descriptor.
     * @param <X> Class of value
     * @return A suitable descriptor.
     */
    public static <X> Value<X> Value(X value) {
    ...
    /**
     * A java variable, could be local, static or field (last not implemented
     * yet)
     *
     */
    public static class Variable extends Value<String> {

        protected Type type;   // java name for type, I, etc

        public Variable(String asmName, Type type) {
            super(asmName);
            this.type = type;
        }
    ...
    /**
     * Convert a java type name to a suitable prefix for a load/store local
     * command. E.g. java type 'I' (integer) maps to 'i', Object types (e.g.
     * Ljava.lang.String; ) map to 'a' rtc
     *
     * @param typeCode
     * @return
     */
    static String asmType(Type type) {
        String typeCode = type.getCode().toLowerCase();  // asm use lower case, java types use upper.
    ...
    public static class Local extends Variable {
        ...
        public Local(String asmName, Type type) {
            super(asmName, type);
            this.prefix = asmType(this.type);
        }

        public Local(String asmName) {
            this(asmName, Type.INT);
        }
    ...
    public static Local Local(String asmName, Type typeName) {
        return new Local(asmName, typeName);
    }

    public static Local Local(String asmName) {
        return new Local(asmName, Type.INT);
    }

    public static class Static extends Variable {

        public Static(String asmName, Type type) {
            super(asmName, type);
    ...
    public static Static Static(String asmName, Type typeName) {
        return new Static(asmName, typeName);
    }

    public static Static Static(String asmName) {
        return new Static(asmName, Type.INT);
    }
